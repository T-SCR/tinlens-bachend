...
export function AIInputWithFile({
  ...
  placeholder = "Paste URL or add details about what to check...",
  minHeight = 52,
  maxHeight = 200,
  accept = "image/*",
  maxFileSize = 5,
  ...
  return (
    <div className={cn("w-full py-2 sm:py-4 px-2 sm:px-0", className)}>
      <div className="relative max-w-full w-full mx-auto flex flex-col gap-2">
        {fileName && <FileDisplay fileName={fileName} onClear={clearFile} />}

        <div className="relative">
          <div
            className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 flex items-center justify-center h-7 sm:h-8 w-7 sm:w-8 rounded-lg bg-black/5 dark:bg-white/5 hover:cursor-pointer hover:bg-black/10 dark:hover:bg-white/10 transition-colors"
          ...
          <Textarea
            ...
            className={cn(
              "w-full bg-black/5 dark:bg-white/5 rounded-2xl sm:rounded-3xl pl-10 sm:pl-12 pr-12 sm:pr-16",
          ...
          <button
            ...
            className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 rounded-xl bg-black/5 dark:bg-white/5 py-1 px-1 hover:bg-black/10 dark:hover:bg-white/10 transition-colors"
            type="button"
          >
            <CornerRightUp
              className={cn(
